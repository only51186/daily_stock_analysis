...
class AutoScheduler:
    ...
    def run_data_validation(self) -> TaskResult:
        ...
        try:
            ...
            # 检查股票数据
            stock_query = "SELECT * FROM stock_daily LIMIT 5000"
            stock_df = pd.read_sql_query(stock_query, conn)
            stock_count = len(stock_df)
            
            # 检查板块数据
            sector_query = "SELECT * FROM sector_data LIMIT 100"
            ...
            logger.info(f"股票数据：{stock_count}只")
            logger.info(f"板块数据：{sector_count}个")
            
            # 数据完整性判断
            if stock_count >= 1000 and sector_count >= 30:
                success = True
                message = f"数据完整性校验通过：股票{stock_count}只，板块{sector_count}个"
            else:
                success = False
                message = f"数据完整性校验失败：股票{stock_count}只，板块{sector_count}个"
            ...
            return TaskResult('data_validation', success, message, 
                            {'stock_count': stock_count, 'sector_count': sector_count}, 
                            duration)

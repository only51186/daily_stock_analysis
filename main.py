...
def generate_visual_report(analysis_results, output_path="stock_analysis_report.html"):
    ...
    try:
        ...
        for item in stock_table_data:
            # 单独处理每一行的数值格式化
            price_text = f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{item['price']:.2f}" if item['price'] is not None else "-"
            change_text = f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{f"{item['change']:.2f}" if item['change'] is not None else "-"
